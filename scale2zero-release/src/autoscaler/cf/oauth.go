...
func (c *client) getUserScopeEndpoint(userToken string) (string, error) {
	...
	userScopeEndpoint := c.goCFClient.Endpoint.TokenEndpoint + "/check_token?" + parameters.Encode()
	return userScopeEndpoint, nil
}

func (c *client) getSpaceDeveloperPermissionEndpoint(userId string, appId string) string {
	parameters := url.Values{}
	parameters.Add("app_guid", appId)
	parameters.Add("developer_guid", userId)
	spaceDeveloperPermissionEndpoint := c.config.API + "/v2/users/" + userId + "/spaces?" + parameters.Encode()
	return spaceDeveloperPermissionEndpoint
}

func (c *client) getUserInfoEndpoint() string {
	return c.goCFClient.Endpoint.TokenEndpoint + "/userinfo"
}
